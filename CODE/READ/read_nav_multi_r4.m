...
% ||| BDS iono model coefficients are not read-in
% ||| only first data record of broadcast iono models are considered
...
% count number of messages for each GNSS
no_eph_gps = sum(contains(NAV, '> EPH G'));
no_eph_glo = sum(contains(NAV, '> EPH R'));
...
% loop over lines
while i < n_NAV         % loop from END OF HEADER to end of file
    i = i + 1;
    line = NAV{i};
    ...
    switch line(1:5)
        ...
        case '> ION'    % Global/Regional ionospheric model parameters
            switch line(7)
                case 'G'        % Klobuchar
                    [klob, i] = save_klob_coeff(klob, NAV, i);
                    
                case 'E'        % NeQuick-G
                    [nequ, i] = save_nequ_coeff(nequ, NAV, i);
                    
                case 'C'        % BDGIM
                    % ||| implement BDGIM model
                    
                otherwise
                    continue
            end
            
        case '> STO'    % System Time and UTC proxy offset parameters
            % ||| consider somehow
            continue
            
        case '> EOP'    % Earth Orientation Parameters
            % ||| consider somehow
            continue
...
% read Klobuchar coefficients
if ~isempty(klob)
    return      % currently only first data record is considered
end
% first line: 
i = i + 1; line = NAV{i};
val_1 = textscan(line,'%f'); val_1 = val_1{1};    
alpha(1) = val_1(7); 
alpha(2) = val_1(8); 
alpha(3) = val_1(9);
% second line:
...
% third line:
i = i + 1; line = NAV{i};
val_3 = textscan(line,'%f'); val_3 = val_3{1};    
beta(4) = val_3(1);
% put together alpha and beta coefficients
klob = [alpha; beta];



function [nequ, i] = save_nequ_coeff(nequ, NAV, i)
% read NeQuick-G coefficients
if ~isempty(nequ)
    return      % currently only first data record is considered
end
% first line: 
