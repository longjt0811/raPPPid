...
for i_sat = 1:num_sat
    ...
    if settings.ADJ.weight_bore
        ...
        Los_Body = ECEF2RWS(X_rot, Epoch.time, Adjust.param_pred(1:6), obs.leap_sec);
        Ant_BoreSight = [0; 0; -1];
    ...
    % For IGSO satellites (e.g., BeiDou) it is in the order of 70 ps or 2.3 cm.
    % check [17], p.564
    shapiro = 0;
    ...
    if settings.OTHER.bool_eclipse && ~settings.ORBCLK.bool_obx
        ...
        if cos_phi < 0 && (norm(X_rot)*sqrt(1-cos_phi^2)) < Const.RE
            if mod(floor(Epoch.gps_time), 600) == 0 && ~settings.INPUT.bool_parfor
                fprintf('Eclipsing Satellite PRN %.0f \t                \n', prn)
            end
            exclude = true;              % eliminate satellite
            status(:) = 13;
        end
    ...
    if settings.OTHER.bool_rec_pco        % Receiver Phase Center Offset is enabled
        ...
        dX_PCO_REC_ECEF_corr = Convert2ProcFrqs(settings, j, los_pco_r, f1, f2, f3, dX_PCO_REC_ECEF_corr);
    end
    ...
    if settings.OTHER.bool_sat_pco && (settings.ORBCLK.bool_sp3 || bool_stream_COM)
        ...
        % convert to the processed frequencies
        dX_PCO_SAT_ECEF_corr = Convert2ProcFrqs(settings, j, los_pco_s, f1, f2, f3, dX_PCO_SAT_ECEF_corr);
    end
    ...
    if settings.OTHER.bool_rec_pcv
        if ~isempty(PCV_rec)    % check if PCV corrections are existing at all (in ANTEX)
            [dX_PCV_rec, ~] = calc_PCV_rec(PCV_rec, j, elev, az, settings, f1, f2, f3);
        end
    end
    ...
    if settings.OTHER.bool_sat_pcv && (settings.ORBCLK.bool_sp3 || bool_stream_COM) && ~exclude
        [dX_PCV_sat, ~] = calc_PCV_sat(PCV_sat, SatOr_ECEF, los0, j, settings, f1, f2, f3, X_rot, pos_XYZ);
    end
